...
void PluginHost::prepare(double sr, int bs)
{
    ...
    block = juce::jmax(1, bs);
    const juce::ScopedLock l(mutex);
    for (auto* p : chain) p->prepareToPlay(rate, block);
}

void PluginHost::release()
{
    const juce::ScopedLock l(mutex);
...
void PluginHost::process(juce::AudioBuffer<float>& b, juce::MidiBuffer& m)
{
    juce::ScopedTryLock l(mutex);
    if (!l.isLocked()) return;
    for (auto* p : chain) p->processBlock(b, m);
}
...
{
    const juce::ScopedLock l(mutex);
    juce::Array<PluginInfo> out;
    for (const auto& d : known.getTypes()) {
        PluginInfo i;
        i.name = d.name; i.vendor = d.manufacturerName;
        i.format = d.pluginFormatName; i.file = d.fileOrIdentifier;
        out.add(i);
    }
    return out;
}
...
juce::Array<PluginHost::PluginInfo> PluginHost::search(const juce::String& q, int maxResults) const
{
    auto all = list();
    juce::Array<PluginInfo> out;
    const auto needle = q.toLowerCase().trim();
    for (const auto& i : all) {
        if (needle.isEmpty() || (i.name + " " + i.vendor + " " + i.format).toLowerCase().contains(needle)) {
            out.add(i);
...
bool PluginHost::load(const juce::String& nameOrPath, juce::String& error)
{
    ...
    bool found = false;
    for (const auto& d : known.getTypes())
        if (d.name.equalsIgnoreCase(nameOrPath) || d.fileOrIdentifier == nameOrPath) { chosen = d; found = true; break; }
...
{
    const juce::ScopedLock l(mutex);
    if (chain.isEmpty()) { error = "No hosted plugin"; return false; }
    auto* p = chain.getLast();

    for (auto* param : p->getParameters())
    {
        const bool nameMatches = param->getName(80).equalsIgnoreCase(parameterName);

        // getParameterID() NÃO existe em AudioProcessorParameter (é da subclasse
        // AudioProcessorParameterWithID). Obtemos o ID via cast, com fallback
        // para o índice numérico — que cobre qualquer parâmetro hospedado.
        auto* withId = dynamic_cast<juce::AudioProcessorParameterWithID*>(param);
        const bool idMatches = (withId != nullptr && withId->paramID.equalsIgnoreCase(parameterName))
                            || juce::String(param->getParameterIndex()).equalsIgnoreCase(parameterName);
    ...
    error = "Parameter not found: " + parameterName;
    return false;
}
...
juce::var PluginHost::state() const
{
    ...
    for (auto* p : chain) {
        ...
        o->setProperty("parameters", p->getNumParameters());
        a.add(o);
    }
    root->setProperty("plugins", a);
    return root;
}